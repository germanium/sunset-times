...
t = gettext.translation('crepyscule_tools', os.getcwd() + '/locale')
_ = t.gettext
...
def get_dictionary(sValue, fLat, fLon, ctime, fUTC=None, sSummerTime=""):
    ...
    if sValue not in lPossibleValues:
        sError = sValue + _(" is not a good value for get_dictionary.") +\
                 _("Possible values are:") + str(lPossibleValues)
...
#    sSunriseHeader = string.join((_('"Sunrise for'), str(fLat1), str(fLon1), \
#                                  str(fUTC1) , str(sSummerTime1) + '"'))
#    sSunsetHeader = string.join((_('"Sunset for'), str(fLat1), str(fLon1), \
#                                 str(fUTC1),  str(sSummerTime1) + '"'))
#    sDaylengthHeader = string.join((_('"Day length for'), str(fLat1), str(fLon1), \
#                                    str(fUTC1),  str(sSummerTime1) + '"'))    
    ...
    if lSunrise2 != None:
        ...
        sSunsetHeader = "Sunset"
        sDaylengthHeader = "Daylength"
#        sSunriseHeader = string.join((_('"Sunrise for'), str(fLat2), str(fLon2), \
#                         str(fUTC2) , str(sSummerTime2) + '"'))
#        sSunsetHeader = string.join((_('"Sunset for'), str(fLat2), str(fLon2), \
#                        str(fUTC2),  str(sSummerTime2) + '"'))
#        sDaylengthHeader = string.join((_('"Day length for'), str(fLat2), str(fLon2), \
#                                        str(fUTC2),  str(sSummerTime2) + '"'))
...
def get_sun_altitude_as_csv(lDateISO8601, fLat1, lAltitude1,\
    ...
    sDate = "Date"
    sHeader = string.join((_('"Sun altitude for'), str(fLat1) + '"'))
    sHeader = string.join((sDate, sHeader), sep=',')
    if lAltitude2 != None:
        sHeader2 = string.join((_('"Sun altitude for'), str(fLat2) + '"'))
...
def get_daylight_variation_as_csv(lDateISO8601, lDaylength1, fLat1, \
    ...
    sDate = "Date"
    sHeader = string.join((_('"Daily variation of light for'), str(fLat1), '"'))
    sHeader = string.join((sDate, sHeader), sep=',')
    lVariation1 = get_daylight_variation(lDaylength1)
    if lDaylength2 != None:
        sHeader2 = string.join((_('"Daily variation of light for'), str(fLat2)\
                                , '"'))
        sHeader = string.join((sHeader, sHeader2), sep=',')
...
def get_solar_flux_as_csv(lDateISO8601, fLat1, lFlux1,\
    ...
    # Write the header: "Date", "Sun altitude for XX.xxN",
    sDate = "Date"
    sHeader = string.join((_('"Daily maximal solar flux for'), str(fLat1) + '"'))
    sHeader = string.join((sDate, sHeader), sep=',')
    if lFlux2 != None:
        sHeader2 = string.join((_('"Daily maximal solar flux for'), str(fLat2) +\
                                '"'))
        sHeader = string.join((sHeader, sHeader2), sep=',')
...
def get_twilight_length_as_csv(lDateISO8601, fLat1, lLength1,\
    ...
    # Write the header: "Date", "Sun altitude for XX.xxN",
    sDate = "Date"
    sHeader = string.join((_('"Twilight length for'), str(fLat1) + '"'))
    sHeader = string.join((sDate, sHeader), sep=',')
    if lLength2 != None:
        sHeader2 = string.join((_('"Twilight length for'), str(fLat2) +\
                                '"'))
        sHeader = string.join((sHeader, sHeader2), sep=',')
